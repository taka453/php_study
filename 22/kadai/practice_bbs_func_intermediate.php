...
if ($_SERVER['REQUEST_METHOD']) {
  if (has_name_error($_POST['name'])) {
  ...
  } else if (has_name_error($_POST['comment'])) {
    $error_message[] = has_commnent_error($_POST['comment']);
  } else if(!count($error_message)) {
    $name = $_POST['name'];
    ...
    $sql = "INSERT INTO bbs(name, date, comment) VALUES('. $implode_array .')";
    insert_db($link, $sql);
  }
}
